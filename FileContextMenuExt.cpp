...
FileContextMenuExt::FileContextMenuExt(void) : m_cRef(1), 
    m_pszMenuText(L"Convert with Filestar"),
    m_pszVerb("convert"),
    m_pwszVerb(L"convert"),
    m_pszVerbCanonicalName("ConvertWithFilestar"),
    m_pwszVerbCanonicalName(L"ConvertWithFilestar"),
    m_pszVerbHelpText("Convert with Filestar"),
    m_pwszVerbHelpText(L"Convert with Filestar"),
	m_nFiles(0)
{
...
void FileContextMenuExt::OnVerbDisplayFileName(HWND hWnd)
{
	...
	for (auto const& x : m_fMap)
	{
		...
		//auto p= x.second.get();
		x.second.wait();
		auto res = x.second.get();
		auto path =  ((FileInfo&)res).GetPath();
		cmd += L"\"" +path + x.first + L"\" ";
		//x.second.
		//x.second._Get_value();
		//((std::_Associated_state<FileInfo>*)((std::_Packaged_state<FileInfo __cdecl(void)>*) & (*x.second._Assoc_state)))->_Result.path
		//memcpy(BUFF, x.first.c_str(), 1);
	}
	
	
	long long sz = cmd.length() + 1;
	WCHAR* BUFF = new WCHAR[sz];
	swprintf_s(BUFF, sz, L"%s", cmd.c_str());
